                        ...
                        <table id="" class="table table-hover table-sort">
                            ...
                            <tbody>                                    
                                @foreach ($users as $user)
                                    ...
                                    <tr data-id=" {{ $user->id }} " data-href="{{ route( 'moderator.clients.show', $user->id) }}" class="{{ $switch_bcg}}">

                                        <td>
                                            @if($user->isBanned == 0)
                                                <form class="bo-ban" method="POST" action="{{ route('moderator.banning', $user->id) }}">
                                            ...
                                            @else
                                                <form class="bo-ban" method="POST" action="{{ route('moderator.unban', $user->id) }}" >
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                                    <input type="hidden" name="_method" value="PUT" />
                                        ...
                                        <td>
                                            <form class="bo-del" method="POST" action="{{ route( 'moderator.clients.destroy', $user->id) }}">
